...
Window::Window(unsigned int width, unsigned int height) {
	...
	gbuffer_shader = { "Shaders/vertex/gbuffer.vert", "Shaders/fragment/gbuffer.frag" };

	gbuffer_lighting_shader = { "Shaders/vertex/light_gbuffer.vert", "Shaders/fragment/light_gbuffer.frag" };


	gbuffer_shader_model = { "Shaders/vertex/gbuffer_model.vert", "Shaders/fragment/gbuffer_model.frag" };

	ssao_shader = { "Shaders/vertex/framebuffer_shader.vert", "Shaders/fragment/ssao.frag" };
	
	ssao_blur_shader = { "Shaders/vertex/framebuffer_shader.vert", "Shaders/fragment/ssao_blur.frag" };

	pos_matrix_ubo = { 2 * sizeof(glm::mat4) };
	unsigned int shader_id[] = { object_shader.GetID(), instancing_shader.GetID(), lighting_shader.GetID(), stencil_shader.GetID(), discard_shader.GetID(), no_material_shader.GetID(), cubemap_reflection_shader.GetID(), normal_visualizing.GetID(), normal_map_shader.GetID(), gbuffer_shader.GetID(), gbuffer_shader_model.GetID()};
	...
	grass_tex = { "Textures/grass.png", GL_CLAMP_TO_EDGE, GL_RGBA, GL_SRGB_ALPHA, true };
	window_tex = { "Textures/window_tex.png", GL_CLAMP_TO_EDGE, GL_RGBA, GL_SRGB_ALPHA, true };
	char faces[][100] =
	...
	diffuse_brick_map = { "Textures/red_brickwall.jpg", GL_CLAMP_TO_EDGE, GL_RGB, false };

	black_specular_map = { "Textures/black_texture.png", GL_CLAMP_TO_EDGE, GL_RGBA };

	normal_map_tex = { "Textures/red_brickwall_normal_map.jpg", GL_CLAMP_TO_EDGE, GL_RGB, false };
	...
	Texture* gbuffer_textures[] = { &gbuffer_pos_tex, &gbuffer_normal_tex, &gbuffer_albedospec_tex };
	GLenum internal_format_gbuff_tex[] = { GL_RGBA16F, GL_RGBA16F, GL_RGBA };
	GLenum type_gbuff_tex[] = { GL_FLOAT, GL_FLOAT, GL_UNSIGNED_BYTE };

	int gbuffer_tex_amount = sizeof(gbuffer_textures) / sizeof(gbuffer_textures[0]);
	gbuffer = { width, height, gbuffer_textures, gbuffer_tex_amount, internal_format_gbuff_tex, type_gbuff_tex, GL_RGBA };
	...
	// get random vectors inside hemisphere(Z doesn't change)
	...
	ssao_fbo = { width, height, ssao_fbo_tex, GL_RED, GL_RED };

	ssao_fbo_blur = { width, height, ssao_fbo_blur_tex, GL_RED, GL_RED };

	ssao_noise_tex = {GL_RGBA16F, 4, 4, GL_RGB, GL_FLOAT, ssao_noise};
}
...
void Window::framebuffer_size_callback(GLFWwindow* window, int width, int height) {
	...
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, window_state_map[window]->samples, GL_RGBA16F, width, height, GL_TRUE);
	...
	window_state_map[window]->ssao_fbo_tex.Bind();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED, GL_FLOAT, NULL);
	window_state_map[window]->ssao_fbo_blur_tex.Bind();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, width, height, 0, GL_RED, GL_FLOAT, NULL);

	window_state_map[window]->gbuffer_pos_tex.Bind();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, NULL);
	window_state_map[window]->gbuffer_normal_tex.Bind();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, NULL);
	window_state_map[window]->gbuffer_albedospec_tex.Bind();
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
	window_state_map[window]->gbuffer.BindRenderbuffer();
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height);
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_RENDERBUFFER, window_state_map[window]->gbuffer.GetRenderbufferId());
...
void Window::Go() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		gbuffer_shader.Activate();
		// CUBES____________________________________________________________________________________________________________________________
		gbuffer_shader.setInt("material.texture_diffuse1", diffuse_map.GetId());
		gbuffer_shader.setInt("material.texture_specular1", specular_map.GetId());
		vao_model.Bind();
		glDrawArraysInstanced(GL_TRIANGLES, 0, 36, objects_amount);
		
		gbuffer_shader_model.Activate();
		gbuffer_shader_model.setInt("material.texture_diffuse1", diffuse_brick_map.GetId());
		gbuffer_shader_model.setInt("material.texture_specular1", black_specular_map.GetId());
		gbuffer_shader_model.setInt("material.normal_map1", normal_map_tex.GetId());
		normal_map_vao.Bind();
		glDrawArraysInstanced(GL_TRIANGLES, 0, 36, normal_map_cubes_amount);
		...
		gbuffer_shader_model.Activate();
		gbuffer_shader_model.setInt("material.texture_specular1", black_specular_map.GetId());
		draw_model(gbuffer_shader_model, loaded_model, models_amount);	
		glStencilMask(0xFF);				// writes data in the stencil buffer(stencil buffer cannot be cleared while start's in read only mode)
		glStencilFunc(GL_ALWAYS, 0, 0xFF);	// stencil buffer will always fill with 0s
		...
		for (unsigned int i = 0; i < 64; i++) {
			ssao_shader.setVec3(("samples[" + std::to_string(i) + "]").c_str(), ssao_kernel[i]);
		}

		ssao_shader.setMat4("projection", projection);
		ssao_shader.setFloat("far_plane", far_plane_camera);
		ssao_shader.setInt("g_position", gbuffer_pos_tex.GetId());
		ssao_shader.setInt("g_normal", gbuffer_normal_tex.GetId());
		ssao_shader.setInt("tex_noise", ssao_noise_tex.GetId());
		ssao_shader.setFloat("width", width);
		ssao_shader.setFloat ("height", height);

		framebuffer_vao.Bind();
		glDrawArrays(GL_TRIANGLES, 0, 6);
		ssao_fbo.UnbindFramebuffer();

		// SSAO BLUR
		ssao_fbo_blur.BindFramebuffer();
		glClear(GL_COLOR_BUFFER_BIT);
		ssao_blur_shader.Activate();
		ssao_blur_shader.setInt("ssao", ssao_fbo_tex.GetId());
		framebuffer_vao.Bind();
		glDrawArrays(GL_TRIANGLES, 0, 6);
		ssao_fbo_blur.UnbindFramebuffer();
		...
		post_proc_fbo.BindFramebuffer();
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);

		gbuffer_lighting_shader.Activate();
		gbuffer_lighting_shader.setInt("g_position", gbuffer_pos_tex.GetId());
		gbuffer_lighting_shader.setInt("g_normal", gbuffer_normal_tex.GetId());
		gbuffer_lighting_shader.setInt("g_albedospec", gbuffer_albedospec_tex.GetId());
		gbuffer_lighting_shader.setInt("ssao", ssao_fbo_blur_tex.GetId());

		for (unsigned int i = 0; i < light_amount; i++) {

			//glm::vec3 asdf = glm::vec3(camera.GetViewMatrix() * glm::vec4(point_light_pos[i], 1.0f));
			//gbuffer_lighting_shader.setVec3(("lights[" + std::to_string(i) + "].position").c_str(), glm::vec3(camera.GetViewMatrix() * glm::vec4(point_light_pos[i], 1.0f)));
			gbuffer_lighting_shader.setVec3(("point_lights[" + std::to_string(i) + "].position").c_str(), point_light_pos[i]);
			//gbuffer_lighting_shader.setVec3(("lights[" + std::to_string(i) + "].position").c_str(), asdf);
			gbuffer_lighting_shader.setVec3(("point_lights[" + std::to_string(i) + "].color").c_str(), point_light_colors[i]);
			...
			gbuffer_lighting_shader.setFloat(("point_lights[" + std::to_string(i) + "].linear").c_str(), linear);
			gbuffer_lighting_shader.setFloat(("point_lights[" + std::to_string(i) + "].quadratic").c_str(), quadratic);

			const float max_brightnesss = std::fmaxf(std::fmaxf(point_light_colors[i].r, point_light_colors[i].g), point_light_colors[i].b);
			float radius = (-linear + std::sqrt(linear * linear - 4 * quadratic * (constant - (256.0f / 5.0f) * max_brightnesss))) / (2.0f * quadratic);
			gbuffer_lighting_shader.setFloat(("point_lights[" + std::to_string(i) + "].radius").c_str(), radius);
		}

		for (unsigned int i = 0; i < sizeof(point_shadow_map_id) / sizeof(point_shadow_map_id[0]); i++)
			gbuffer_lighting_shader.setInt(("shadow_map_point[" + std::to_string(i) + "]").c_str(), point_shadow_map_id[i]);

		gbuffer_lighting_shader.setVec3("view_pos", camera_position);
		glm::vec3 dir_light_direction = glm::vec3(-0.2f, -1.0f, -0.3f);
		gbuffer_lighting_shader.setVec3("dir_light.direction", dir_light_direction);
		gbuffer_lighting_shader.setMat4("view", view);
		gbuffer_lighting_shader.setMat4("projection", projection);


		glm::vec3 camera_front = camera.GetCameraFront();
		gbuffer_lighting_shader.setVec3("spot_light.position", camera_position);
		gbuffer_lighting_shader.setVec3("spot_light.direction", camera_front);
		//shader.setFloat("spot_light.constant", 1.0f);
		gbuffer_lighting_shader.setFloat("spot_light.linear", 0.09f);
		gbuffer_lighting_shader.setFloat("spot_light.quadratic", 0.032f);
		glm::vec3 spot_diff_power = spot_light_color * glm::vec3(1.0f);
		glm::vec3 spot_spec_power = spot_light_color * glm::vec3(1.0f);
		gbuffer_lighting_shader.setVec3("spot_light.diffuse_col", spot_diff_power);
		gbuffer_lighting_shader.setVec3("spot_light.spec_col", spot_spec_power);
		gbuffer_lighting_shader.setFloat("spot_light.constant", 1.0f);


		if (flashlight) {
			float flashlight_cut_off = glm::cos(glm::radians(12.5f));
			float flashlight_outer_cut_off = glm::cos(glm::radians(20.0f));
			gbuffer_lighting_shader.setFloat("spot_light.cut_off", glm::cos(glm::radians(12.5f)));
			gbuffer_lighting_shader.setFloat("spot_light.outer_cut_off", glm::cos(glm::radians(20.0f)));
		}
		else {
			float flashlight_cut_off = glm::cos(glm::radians(0.0f));
			float flashlight_outer_cut_off = glm::cos(glm::radians(0.0f));
			gbuffer_lighting_shader.setFloat("spot_light.cut_off", glm::cos(glm::radians(0.0f)));
			gbuffer_lighting_shader.setFloat("spot_light.outer_cut_off", glm::cos(glm::radians(0.0f)));
		}

		gbuffer_lighting_shader.setMat4("light_space_matrix_directional", light_space_matrix);

		gbuffer_lighting_shader.setInt("shadow_map_spot", spot_shadow_fbo_tex.GetId());
		gbuffer_lighting_shader.setInt("shadow_map_direct", directional_shadow_fbo_tex.GetId());
		gbuffer_lighting_shader.setFloat("point_map_far_plane", far_plane_cubemap_shadow);

		gbuffer_lighting_shader.setFloat("shininess", 32.0f);

		glm::vec3 dir_diff_power = dir_light_color * glm::vec3(0.2f);
		glm::vec3 dir_spec_power = dir_light_color * glm::vec3(0.3f);

		gbuffer_lighting_shader.setVec3("dir_light.diffuse_col", dir_diff_power);
		gbuffer_lighting_shader.setVec3("dir_light.spec_col", dir_spec_power);
		...
		//framebuffer_shader.setInt("screen_texture", gbuffer_normal_tex.GetId());
		...
		gbuffer.BindFramebufferR();
		glBindFramebuffer(GL_DRAW_FRAMEBUFFER, post_proc_fbo.GetFramebufferId());
		...
		// MODEL OUTLINE____________________________________________________________________________________________________________________
		glStencilFunc(GL_NOTEQUAL, 1, 0xFF);	// stencil buffer will fill with values which are not equal to 1 (the outlining)
		glStencilMask(0x00);					// doesn't write in the stencil buffer
		glDisable(GL_DEPTH_TEST);				// show object through other pbjects
		stencil_shader.Activate();
		...
		//glReadBuffer(GL_COLOR_ATTACHMENT0);
		//glDrawBuffer(GL_COLOR_ATTACHMENT0);
		//gbuffer.BlitFramebuffer(width, height);
		//glReadBuffer(GL_COLOR_ATTACHMENT1);
		//glDrawBuffer(GL_COLOR_ATTACHMENT1);
		//gbuffer.BlitFramebuffer(width, height);
		//gbuffer.UnbindFramebuffer();
		...
		//framebuffer_shader.setInt("screen_texture", ssao_fbo_blur_tex.GetId());
		framebuffer_shader.setInt("bloom_blur", pingpongFboTexture[!horizontal]->GetId());
		framebuffer_shader.setFloat("exposure", exposure);
		framebuffer_shader.setFloat("near_plane", 0.1f);
		framebuffer_shader.setFloat("far_plane", 100.0f);
