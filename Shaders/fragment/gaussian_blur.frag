...
void main(){
    vec2 texel_size = 1.0f / textureSize(image, 0);

    vec3 result = texture(image, tex_coords).rgb * weight[0];

    if(horizontal){
        for(int i = 1; i < size_of_weight; i++){
            result += texture(image, tex_coords + vec2(texel_size.x * i, 0.0f)).rgb * weight[i];
            result += texture(image, tex_coords - vec2(texel_size.x * i, 0.0f)).rgb * weight[i];
        }
    }
    else{
        for(int i = 1; i < size_of_weight; i++){
            result += texture(image, tex_coords + vec2(0.0f, texel_size.y * i)).rgb * weight[i];
            result += texture(image, tex_coords - vec2(0.0f, texel_size.y * i)).rgb * weight[i];
        }
    }
